...
readonly CASA_EXEC=cassetteos-user-service
readonly CASA_SERVICE=cassetteos-user-service.service
...
if [[ -z "${CASA_CONF}" ]]; then
    CASA_CONF=/etc/cassetteos/user-service.conf
fi

CASA_DB_PATH=$( (grep -i dbpath "${CASA_CONF}" || echo "/var/lib/cassetteos/db") | cut -d'=' -sf2 | xargs )
